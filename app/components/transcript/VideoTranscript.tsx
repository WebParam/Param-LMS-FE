...
const VideoTranscript = ({ handleTimestampClick }: VideoTranscriptProps) => {
  ...
  const token :string = "cca5d0ef803dac4b5746c4a378073e38";
  const videoId : string = "915091193";
    ...
    const fetchTranscript = async () => {
      try {
        ...
        if (
          ...
          parsedTranscript[0].text.includes("WEBVTT -")
        ) {
    ...
    <div>
      {transcript?.map((entry: any, index: number) => (
        <div
          ...
          key={entry?.time}
        >
          <p>{`${formatTime(entry?.time)} - ${formatTime(
