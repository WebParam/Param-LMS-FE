...
function WorkBookModal(props: any) {
  ...
  const submit = () => {
    ...
    if (description === "")
      setDescriptionError("Please enter Workbook description");
    if (titleRef.current?.value === "")
      setTitleError(
        "Please enter the Max Length of 30 characters for Workbook Title"
      );
    if (moduleCodeRef.current?.value === "")
      setModuleCodeError(
        "Please enter the Max Length of 10 characters for Work Experience Module Code/No."
      );
    else {
      props.onHide();
      ...
      >
        <form action={createModuleWithParams}>
          <Modal.Header closeButton>
            <Modal.Title>Create Work Book - {props.title}</Modal.Title>
          </Modal.Header>

          <Modal.Body>
            <div>
              <h5>Work Experience Module Code</h5>
              <input
                name="moduleCode"
...
export default dynamic(() => Promise.resolve(WorkBookModal), {
  ssr: false,
});
